...
def odrerObject(self):
    ...
    self.order1 = [self.objOrderTitle, self.objOrderSendMean1, self.objOrderSendMean2, self.objOrderSendTime1, self.objOrderSendTime2]
    self.order2 = [self.objOrderPayWay1, self.objOrderPayWay2, self.objOrderPayWay3, self.objOrderMoney, self.objOrderSendPay, self.objOrderCommit]


def nameObject(self):
    self.name1 = ['订单确认', '送货上门', '门店自提(免配送费)', '即时送', '定时送']
